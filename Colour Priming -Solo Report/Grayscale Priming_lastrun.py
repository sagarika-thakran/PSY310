...
expName = 'Grayscale Priming'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Sugandha\\OneDrive\\Desktop\\Colour Priming (Control)\\Grayscale Priming_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    introduction_text = visual.TextStim(win=win, name='introduction_text',
        text='"Greetings! \nIn the upcoming task, you will see different objects in varying shades of gray. You will see three grayscale images of the same object. Choose the image that best represents what the object should look like if it were colored correctly. Use your knowledge of the object to guide your choice.\nPress \'y\' if you think that the image is the correct version, and press \'n\' if you do not think so. If, at any point in the experiment you feel the need or want to exit the experiment, please press the Escape button. Press space bar when you\'re ready to proceed."',
        font='Arial',
    ...
    # create starting attributes for key_response
    key_response.keys = []
    key_response.rt = []
    _key_response_allKeys = []
    # keep track of which components have finished
    InstructionsComponents = [introduction_text, key_response]
    ...
    while continueRoutine:
        ...
        if key_response.status == STARTED and not waitOnFlip:
            theseKeys = key_response.getKeys(keyList=['space','y','n'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_key_response_allKeys):
                ...
                key_response.duration = _key_response_allKeys[-1].duration
    ...
    if key_response.keys != None:  # we had a response
        ...
        thisExp.addData('key_response.duration', key_response.duration)
    thisExp.nextEntry()
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('Control group conditions.xlsx'),
    ...
    for thisTrial in trials:
        ...
        image_stim.setImage(stim_image)
        # create starting attributes for Response
        Response.keys = []
        Response.rt = []
        _Response_allKeys = []
        # keep track of which components have finished
        TrialComponents = [fixation, image_stim, Response]
        ...
        while continueRoutine and routineTimer.getTime() < 4.0:
            ...
            if image_stim.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if image_stim.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_stim.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if Response.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Response.tStartRefresh + 3.0-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if Response.status == STARTED and not waitOnFlip:
                theseKeys = Response.getKeys(keyList=['y','n'], ignoreKeys=["escape"], waitRelease=False)
